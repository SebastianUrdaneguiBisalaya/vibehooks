...
<Blockquote.Warning>
	<Blockquote.Content>
		Calling <strong>send()</strong> while <strong>isConnected</strong> is{' '}
		<strong>false</strong> will not queue the message; the operation will fail
		and update the <strong>error</strong> state immediately.
	</Blockquote.Content>
</Blockquote.Warning>

<Blockquote.Tip>
	<Blockquote.Content>
		This hook automatically attempts to parse incoming message data as JSON. If
		the data is not a valid JSON string, it will return the raw message.
	</Blockquote.Content>
</Blockquote.Tip>
