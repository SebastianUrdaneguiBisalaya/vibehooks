...
<Blockquote.Note>
	<Blockquote.Content>
		###### The hook automatically tracks global PiP events, so{' '}<strong>isActive</strong> will stay in sync even if the user exits PiP using the browser's native window controls
	</Blockquote.Content>
</Blockquote.Note>
