import { Blockquote } from '../../../../components/shared/Blockquote';
import CodeBlock from '../../../../components/shared/Codeblock';
import Demo from './Demo';
import Table from '../../../../components/shared/Table';
import data from './data.json';

export const metadata = {
	title: 'useAudio - vibeHooks',
	description:
		'A React hook providing a declarative and unopinionated interface to manage the HTML5 Audio API lifecycle.',
	openGraph: {
		title: 'useAudio - vibeHooks',
		description:
			'A React hook providing a declarative and unopinionated interface to manage the HTML5 Audio API lifecycle.',
	},
};

# useAudio

### A React hook providing a declarative and unopinionated interface to manage the HTML5 Audio API lifecycle.

<Blockquote.Warning>
	<Blockquote.Content>
		###### Browsers often block audio from playing automatically{' '}<strong>(autoplay)</strong> unless the user has interacted with the domain first or the audio is muted.
	</Blockquote.Content>
</Blockquote.Warning>

<Blockquote.Caution>
	<Blockquote.Content>
		###### When the <strong>src</strong> changes, the hook creates a new{' '}<strong>Audio</strong> instance. Ensure you handle state cleanup in your UI if you are switching tracks rapidly to avoid overlapping sounds.
	</Blockquote.Content>
</Blockquote.Caution>

#### Installation

<CodeBlock code={`npm install @vibehooks/react`} language='bash' />

#### Description

The **useAudio** abstracts the complexity of the **HTMLAudioElement**, offering reactive state tracking for audio playback status (playing, paused, loading, etc.) and error handling. It manages the internal audio instance, ensuring that event listeners are correctly cleaned up and audio is paused when the component unmounts to prevent memory leaks or ghost audio.

#### Parameters

<Table data={data.parameters} />

#### Return values

<Table data={data.returnValues} />

#### Demo

<Demo />

#### Source code

```tsx
'use client';

import { useAudio } from '@vibehooks/react';
import { Button } from '@/components/ui/Button';
import { Tag } from '@/components/ui/Tag';
import { Layout } from '@/layouts/Layout';

export default function Demo() {
	const { error, pause, play, status } = useAudio({ src: '/music.mp3' });
	return (
		<Layout>
			<Layout.Title>Audio</Layout.Title>
			<Tag.Primary>Status: {status}</Tag.Primary>
			<Layout.Error>Error: {error?.message}</Layout.Error>
			<div className='flex flex-row items-center gap-2'>
				<Button.Primary onClick={play}>Play</Button.Primary>
				<Button.Secondary onClick={pause}>Pause</Button.Secondary>
			</div>
		</Layout>
	);
}
```
