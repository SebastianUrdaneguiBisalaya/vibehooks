...
<Blockquote.Important>
	<Blockquote.Content>
		Data is serialized using <strong>JSON.stringify()</strong>. Ensure your data
		types are JSON-compatible (avoid using <strong>Map</strong>,{' '}
		<strong>Set</strong>, or objects with circular references).
	</Blockquote.Content>
</Blockquote.Important>

<Blockquote.Tip>
	<Blockquote.Content>
		This hook uses <strong>useSyncExternalStore</strong>, which automatically
		synchronizes the state across all components in your application that share
		the same key.
	</Blockquote.Content>
</Blockquote.Tip>
...
<Blockquote.Note>
	<Blockquote.Content>
		An internal cache is used to prevent redundant <strong>localStorage</strong>{' '}
		reads, optimizing performance during frequent re-renders.
	</Blockquote.Content>
</Blockquote.Note>
