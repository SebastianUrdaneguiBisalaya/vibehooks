...
<Blockquote.Warning>
	<Blockquote.Content>
		Browser support is strictly limited to Google Chrome. Brave and Edge are
		explicitly filtered out due to inconsistent Speech API implementations.
	</Blockquote.Content>
</Blockquote.Warning>

<Blockquote.Important>
	<Blockquote.Content>
		The <strong>start()</strong> method must be triggered by a user gesture
		(e.g., a click event) to comply with browser security policies regarding
		microphone access.
	</Blockquote.Content>
</Blockquote.Important>
...
<Blockquote.Note>
	<Blockquote.Content>
		The hook includes an auto-restart mechanism for the{' '}
		<strong>no-speech</strong> error to keep the session alive unless{' '}
		<strong>stop()</strong> is called manually.
	</Blockquote.Content>
</Blockquote.Note>
