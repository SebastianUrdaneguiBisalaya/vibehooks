...
<Blockquote.Tip>
	<Blockquote.Content>
		###### To allow users to select multiple files, ensure you pass the{' '}<strong>multiple: true</strong> option. Otherwise, the hook will default to a single-file selection.
	</Blockquote.Content>
</Blockquote.Tip>

<Blockquote.Note>
	<Blockquote.Content>
		###### This hook provides the state and logic for file handling but does not perform the actual upload. You should use the <strong>files</strong> array to build a <strong>FormData</strong> object for your API calls.
	</Blockquote.Content>
</Blockquote.Note>
